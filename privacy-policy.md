...
services, and all of that collection is **off by default**.

Nothing is sent until you switch "Analytics" on in Settings: before that no analytics SDK is
initialised, and on the website no counter script is loaded at all. Switching it back off stops new
events; on the website the counter that is already loaded stops sending data and is gone completely
after the page is reloaded.

While analytics is on, the App sends these events and nothing else:
...
- **Google, Firebase Crashlytics** (Android). On a crash it sends the stack trace, the device model,
  the OS and application version and an installation identifier. Automatic collection is disabled in
  the manifest and is only enabled once you switch Analytics on.
- **Google, Firebase Performance Monitoring** (Android). Measures startup time, screen rendering and
  the duration of the App's own network requests. Same switch, same default.
- **Yandex, Yandex.Metrica** (website). Sets its own cookie and collects the IP address, user agent,
  ...
  counter's configuration, so with analytics on your interaction with the page is recorded.
...
- settings — language, theme, accent colour, and the analytics consent flag (`analytics.enabled`,
  absent or `false` until you turn it on);
...
The App uses the analytics services listed in section 1, and only after you consent to them, plus the
services listed in section 3, which you trigger yourself. There are no others. The source code is
...
information. With analytics switched on, Google and Yandex receive the device and session data listed
in section 1 regardless of the user's age; leaving analytics off prevents that.
...
Analytics can be switched off in Settings at any moment; after that the App sends nothing. Events
that were already sent are stored by Google and Yandex under their own terms — request access or
deletion from them directly through the links in section 1. In the browser you can also clear the
site's cookies and `localStorage`, which removes the Yandex.Metrica identifier.
