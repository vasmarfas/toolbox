...
object Currency {
    private const val URL = "https://open.er-api.com/v6/latest/USD"
    ...
    suspend fun fetch(): CurrencyRates {
        val text = Net.client.get(URL).bodyAsText()
        val parsed = parse(text) ?: throw IllegalStateException(getString(Res.string.unexpected_response))
        val now = currentEpochMillis()
        Prefs.store.put(RATES_KEY, text)
        Prefs.store.put(TIME_KEY, now.toString())
        return CurrencyRates(parsed, now)
    }
