...
private fun VoltageDividerScreen() {
    ...
    DividerField(vinText, { vinText = it }, "Vin", Res.string.unit_v.str(), vin)
    if (target != DividerTarget.VOUT) DividerField(voutText, { voutText = it }, "Vout", Res.string.unit_v.str(), vout)
    if (target != DividerTarget.R1) DividerField(r1Text, { r1Text = it }, "R1", Res.string.unit_ohm.str(), r1)
    if (target != DividerTarget.R2) DividerField(r2Text, { r2Text = it }, "R2", Res.string.unit_ohm.str(), r2)
    ...
    if (vin != null && solved != null) {
        ...
        ResultCard {
            KeyValueRow("Vout", Si.format(outVoltage, Res.string.unit_v.str()))
            KeyValueRow("R1", Si.format(res1, Res.string.unit_ohm.str()))
            KeyValueRow("R2", Si.format(res2, Res.string.unit_ohm.str()))
            KeyValueRow(Res.string.current.str(), Si.format(current, Res.string.unit_a.str()))
            KeyValueRow(Res.string.power_in_r1.str(), Si.format(current * current * res1, Res.string.unit_w.str()))
            KeyValueRow(Res.string.power_in_r2.str(), Si.format(current * current * res2, Res.string.unit_w.str()))
        ...
        if (target != DividerTarget.VOUT) {
            ...
            ResultCard(Res.string.nearest_e24_value.str()) {
                KeyValueRow(if (target == DividerTarget.R1) "R1" else "R2", Si.format(standard, Res.string.unit_ohm.str()))
                KeyValueRow(Res.string.vout_with_it.str(), Si.format(actual, Res.string.unit_v.str()))
            }
        }
    }
}
