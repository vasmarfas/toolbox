...
val deviceInfoTool = Tool(
    ...
    description = Res.string.os_model_hardware_locale_time_zone_screen_se,
...
private fun DeviceInfoScreen() {
    ...
    ResultCard(title = Res.string.system_2.str()) {
    ...
    ResultCard(title = Res.string.sensors.str()) {
        ...
        else Text(sensors.joinToString(", ") { it.name.lowercase().replace('_', ' ') }, style = MaterialTheme.typography.bodyMedium)
    }
    if (NetCapabilities.interfaces) {
        ResultCard(title = Res.string.network.str()) {
            networkInterfaces().filter { it.isUp && !it.isLoopback && it.addresses.isNotEmpty() }.forEach { nif ->
...
val batteryTool = Tool(
    ...
    description = Res.string.charge_level_state_temperature_voltage_curre,
...
private fun BatteryScreen() {
    ...
    val b = info
    if (b == null) {
        Text(Res.string.no_battery_information_is_available_on_this.str())
        return
    }
    b.level?.let { level ->
    ...
    ResultCard {
        b.charging?.let { KeyValueRow(Res.string.charging_2.str(), if (it) Res.string.yes.str() else Res.string.no.str(), mono = false, copyable = false) }
        b.details.forEach { (k, v) -> KeyValueRow(k, v, mono = false, copyable = false) }
    }
}
...
val displayInfoTool = Tool(
    ...
    description = Res.string.window_size_in_px_and_dp_density_font_scale,
...
private fun DisplayInfoScreen() {
    ...
    ResultCard {
        ...
        extras.forEach { (k, v) -> KeyValueRow(k, v, mono = false, copyable = false) }
    }
}
...
val clipboardTool = Tool(
    ...
    description = Res.string.show_what_is_in_the_clipboard_as_text_with_l,
...
private fun ClipboardScreen() {
    ...
    text?.let { t ->
        ResultCard {
            KeyValueRow(Res.string.characters_2.str(), t.length.toString(), copyable = false)
