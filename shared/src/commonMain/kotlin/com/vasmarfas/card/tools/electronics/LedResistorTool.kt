...
private fun LedResistorScreen() {
    ...
    ChoiceChips(
        ...
        label = { "${it.title.str()} ${it.forward.fmt(1)} ${Res.string.unit_v.str()}" },
    )
    NumberField(
        value = currentText,
    ...
    if (supply != null && forward != null && current != null && count != null) {
        ...
        if (result == null) {
            ErrorText(Res.string.led_supply_voltage_must_exceed.str())
        } else {
            ResultCard {
                KeyValueRow(Res.string.exact_resistance.str(), Si.format(result.resistance, Res.string.unit_ohm.str()))
                KeyValueRow(Res.string.nearest_e12_not_below.str(), "${Si.format(result.e12, Res.string.unit_ohm.str())} → ${result.currentE12Ma.fmt(1)} ${Res.string.unit_ma.str()}")
                KeyValueRow(Res.string.nearest_e24_not_below.str(), "${Si.format(result.e24, Res.string.unit_ohm.str())} → ${result.currentE24Ma.fmt(1)} ${Res.string.unit_ma.str()}")
                KeyValueRow(Res.string.resistor_power.str(), Si.format(result.power, Res.string.unit_w.str()))
                KeyValueRow(Res.string.recommended_rating.str(), "${result.ratingW.fmt(3)} ${Res.string.unit_w.str()}")
                KeyValueRow(Res.string.voltage_across_resistor.str(), "${(supply - forward * count).fmt(2)} ${Res.string.unit_v.str()}")
            }
        }
    }
}
