...
val passwordStrengthTool = Tool(
    ...
    description = Res.string.entropy_estimate_with_penalties_for_sequence,
...
private fun PasswordStrengthScreen() {
    ...
    Text(
        Res.string.the_password_is_analysed_on_this_device_and.str(),
    ...
    ResultCard {
        ...
        KeyValueRow(Res.string.entropy.str(), "${result.bits.fmt(1)} ${Res.string.bits_2.str()}", copyable = false)
        ...
        if (result.rawBits - result.bits > 0.5) {
            KeyValueRow(
                ...
                "−${(result.rawBits - result.bits).fmt(1)} ${Res.string.bits_2.str()}",
                copyable = false,
            )
        }
    }
    ...
    if (result.issues.isNotEmpty()) {
        ResultCard(Res.string.detected_patterns.str()) {
            result.issues.forEach { issue ->
                KeyValueRow(issue.text.str(), "−${issue.penaltyBits.fmt(1)} ${Res.string.bits_2.str()}", mono = false, copyable = false)
            }
        }
    }
