...
val batteryCalculatorTool = Tool(
    ...
    description = Res.string.runtime_under_a_load_in_ma_or_w_charging_tim,
    ...
    when (mode) {
        BatteryMode.RUNTIME -> {
            ...
            NumberField(
                ...
                suffix = if (loadKind == LoadKind.CURRENT) "mA" else "W",
            ...
            if (capacity != null && load != null && efficiency != null && (loadKind == LoadKind.CURRENT || voltage != null)) {
                ...
                ResultCard {
                    ...
                    if (voltage != null) KeyValueRow(Res.string.energy.str(), "${Battery.mahToWh(capacity, voltage).fmt(2)} Wh")
                }
            }
        }
        BatteryMode.CHARGE -> {
        ...
        BatteryMode.ENERGY -> {
            ...
            NumberField(
                ...
                suffix = if (direction == EnergyDirection.MAH_TO_WH) "mAh" else "Wh",
                isError = energyText.isNotBlank() && energy == null,
            )
            if (energy != null && voltage != null) {
                ResultCard {
                    if (direction == EnergyDirection.MAH_TO_WH) {
                        KeyValueRow("Wh", Battery.mahToWh(energy, voltage).fmt(2))
                    } else {
                        KeyValueRow("mAh", Battery.whToMah(energy, voltage).fmt(0))
                    }
                    KeyValueRow(Res.string.at_voltage.str(), "${voltage.fmt(2)} V", copyable = false)
                }
            }
        }
    }
