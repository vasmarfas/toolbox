...
val unitPriceTool = Tool(
    ...
    description = Res.string.unit_price_description,
                ...
                Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxWidth()) {
                        ...
                        DropdownChoice(
                            ...
                            label = Res.string.unit_price_unit.str(),
                            text = { it.label.str() },
                            modifier = Modifier.weight(1f),
                        )
                    }
                    if (result != null) {
                        val perHundred = result.dimension.hundredLabel?.let { " · ${(result.perBase / 10).fmt(2)} / 100 ${it.str()}" }.orEmpty()
                        val extra = best[result.dimension]?.let { UnitPrice.extraPercent(result.perBase, it.perBase) } ?: 0.0
                        Text(
                            text = "${result.perBase.fmt(2)} / ${result.dimension.baseLabel.str()}$perHundred" + when {
