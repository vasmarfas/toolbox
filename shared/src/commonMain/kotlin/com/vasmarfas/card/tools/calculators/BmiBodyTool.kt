...
val bmiBodyTool = Tool(
    ...
    description = Res.string.bmi_with_who_category_bmr_mifflin_st_jeor_da,
...
private fun BmiBodyScreen() {
    ...
    fun weight(kg: Double): String = if (metric) "${kg.fmt(1)} kg" else "${(kg / KG_PER_LB).fmt(1)} lb"
    ...
    val lengthUnit = if (metric) "cm" else "in"
    SegmentedChoice(
    ...
    if (heightCm != null && weightKg != null) {
        ...
        if (age != null) {
            ...
            ResultCard(Res.string.daily_energy.str()) {
                KeyValueRow(Res.string.bmr_mifflin_st_jeor.str(), "${bmr.fmt(0)} kcal")
                KeyValueRow(Res.string.tdee_with_activity.str(), "${tdee.fmt(0)} kcal")
                KeyValueRow(Res.string.weight_loss_15.str(), "${(tdee * 0.85).fmt(0)} kcal")
                KeyValueRow(Res.string.weight_gain_15.str(), "${(tdee * 1.15).fmt(0)} kcal")
            }
        }
